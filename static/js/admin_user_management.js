...
function suspendUser(userId) {
    fetch(`http://localhost:8089/users/${userId}/status?status=suspended`, { method: 'PUT' })
        .then(() => {
...
function banUser(userId) {
    fetch(`http://localhost:8089/users/${userId}/status?status=banned`, { method: 'PUT' })
        .then(() => {
            alert('User banned!');
            loadUsers(); // Reload user list
        })
...
function deleteUser(userId) {
    fetch(`http://localhost:8089/users/${userId}`, { method: 'DELETE' })
        .then(() => {
            alert('User deleted!');
            loadUsers(); // Reload user list
        })
